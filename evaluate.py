...
def evaluate(input_lang, output_lang, pairs,encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
...
def evaluateRandomly(input_lang, output_lang, pairs,encoder, decoder, n=10):
    for i in range(n):
        ...
        print('>', pair[0])
        print('=', pair[1])
        output_words, attentions = evaluate(input_lang, output_lang, pairs,encoder, decoder, pair[0])
...
if __name__=='__main__':
    ...
    attn_decoder1_sd=torch.load('../model/attn_decoder1.pt', map_location='cpu')
